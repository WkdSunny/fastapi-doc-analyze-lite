...
# @shared_task
# def wait_for_celery_task(task_id: str, timeout: int):
#     """
#     Wait for a Celery task to complete within a given timeout.
...
#     """
#     task = AsyncResult(task_id)
#     start_time = asyncio.run(asyncio.sleep(0))  # Initialize the event loop
    
#     try:
#         while not task.ready():
#             elapsed_time = asyncio.run(asyncio.sleep(0)) - start_time
#             if elapsed_time > timeout:
#                 raise TimeoutError(f"Celery task {task_id} timed out after {timeout} seconds")
...
#         result = task.result
#         if task.failed():
#             raise Exception(f"Task failed with exception: {result}")  # task.result contains the exception traceback
#         logger.info(f"Task {task_id} completed successfully with result: {result}")
#         return result
